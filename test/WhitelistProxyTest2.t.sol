...
import {WhitelistProxy2} from "../src/WhitelistProxy2.sol";

interface Proxy {
    function upgradeToAndCall(address newImplementation, bytes calldata data) external;
}

contract WhitelistProxy2Test is Test {
    ...
    function setUp() public {
        ...
        // Upgrade proxy
        WhitelistProxy2 whitelistProxy = new WhitelistProxy2();
        vm.prank(admin);
        Proxy(USDC).upgradeToAndCall(
            address(whitelistProxy), abi.encodeWithSignature("initializeWhitelistProxy(address)", whitelister)
        );
    }
    ...
    function testNormalUserCannotTransfer() public {
        ...
        vm.expectRevert("You are not invited.");
        ERC20(USDC).balanceOf(user2);
    }

    function testWhiteListUserCanTransfer() public {
        vm.prank(whitelister);
        WhitelistProxy2(payable(USDC)).addWhitelist(user1);
        assertEq(WhitelistProxy2(payable(USDC)).isWhitelisted(user1), true);
