...
import {WhitelistProxy} from "../src/WhitelistProxy.sol";

interface Proxy {
    function upgradeToAndCall(address newImplementation, bytes calldata data) external;
}

contract WhitelistProxyTest is Test {
    ...
    function setUp() public {
        ...
        // Upgrade proxy
        WhitelistProxy whitelistProxy = new WhitelistProxy();
        vm.prank(admin);
    ...
    function testWhiteListUserCanTransfer() public {
        ...
        WhitelistProxy(payable(USDC)).addWhitelist(user1);
        assertEq(WhitelistProxy(payable(USDC)).isWhitelisted(user1), true);
